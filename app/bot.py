...
def run_polling():
    # Defensive: make sure *this* thread has an event loop explicitly
    # set before handing control to python-telegram-bot. Some loop
    # policies (e.g. uvloop's, if something upstream installs it
    # process-wide) do not auto-create a loop the way stdlib asyncio
    # does, and raise "There is no current event loop in thread
    # 'MainThread'" otherwise.
    try:
